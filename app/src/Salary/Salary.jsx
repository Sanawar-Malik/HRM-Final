...
import { allSalary } from '../services/salarySlice';
...
const Salary = () => {
  ...
  const salaryRecords = salaries.salaries || [];
    ...
    <>
      <div className="mx-auto bg-white pb-4 mt-20 shadow-2xl rounded-md w-11/12">
        <div className="flex justify-between w-full p-4  ">
          <h1 className="ml-3 text-2xl font-bold">Salary</h1>
          <div>
            ...
            {showModal && <AddSalary setShowModal={setShowModal} />}
          </div>
        </div>
        ...
        <div className="mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
          <div className="inline-block min-w-full shadow rounded-lg overflow-hidden">
            <table className="min-w-full leading-normal">
              ...
              <tbody>
                {Array.isArray(salaryRecords) ? salaryRecords.map((salary) => {
                  const employee = employees.find((a) => a.id === salary.employee);
                  return (
                    <tr key={salary.id}>
                      ...
                      <td className="px-5 py-2 border-b border-gray-200 bg-white ">
                        <p className="text-black whitespace-no-wrap">{salary.allowance}</p>
                      </td>

                      <td className="px-5 py-2 border-b border-gray-200 bg-white ">
                        <p className="text-black whitespace-no-wrap">{salary.medical}</p>
                      </td>

                      <td className="px-5 py-2 border-b border-gray-200 bg-white ">
                        <p className="text-black whitespace-no-wrap">{employee ? employee.first_name : 'Unknown'}</p>
                      </td>
                      <td className="px-5 py-2 border-b border-gray-200 bg-white ">
                        <div className="flex gap-4">
                          <Tooltip title="Edit" onClick={() => [setId(salary.id), setUpdateModal(true)]}>
                            <IconButton >
                              <EditIcon />
                            </IconButton>
                          </Tooltip>
                          {UpdateModel && <UpdateSalary id={id} setUpdateModal={setUpdateModal} />}

                          <Tooltip title="Delete">
                            <IconButton onClick={() => [setId(salary.id), setDeleteModal(true)]}>
                              <DeleteIcon />
                            </IconButton>
                          </Tooltip>
                          {DeleteModal && <DeleteSalary id={id} setDeleteModal={setDeleteModal} />}
